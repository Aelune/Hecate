...
# Lazy load NVM (user installation path)
...
#fdcd() {
#  local dir
#  dir=$(find "$HOME" -type d -not -path '*/\.*' -maxdepth 5 2>/dev/null | fzf --height=40% --reverse --border --prompt="📁 Select directory: ") && cd "$dir"
#}
#alias j='fdcd'
cdf() {
  local dir
